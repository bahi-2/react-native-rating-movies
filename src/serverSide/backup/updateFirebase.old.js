...
async function getAllMovies() {
    ...
    for (day = 0; day < 7; day++) {
        for (cinema in cinemaLinks) {
            ...
            let movies = await getMovieItems(cinemaLink, day);
            for (movie of movies) {
                if (!distinctTitles.has(movie.title)) {
                    allMovies.push(movie);
                    distinctTitles.add(movie.title);
                }
            }
        }
    }
...
async function updateFirebaseSchedule() {
    ...
    for (day in moviesByDay) {
        ...
        for (movieItem of movieItems) {
            ...
            let newRef = dayRef.child(cinemaName).child(movieItem.title);
            await newRef.set({
                schedule: movieItem.schedule,
                cinestarLink: movieItem.cinestarLink,
                ...allMovies[movieItem.title]
            });
        }
    }
    return true;
...
async function updateFirebaseMovies() {
    ...
    for (movieItem of movieItems) {
        let newRef = moviesRef.child(movieItem.title);
        await newRef.set({
...
function scrapeIMDBId(movieItem, year = null) {
    ...
    return fetch(encodeURI(FULL_URL))
        ...
        .catch(err => {
            console.log(`Error while scraping IMDB id for ${movieItem.title}: ${err}`);
            return null;
        });
}
...
function getIMDBIdApi(movieItem, year = null) {
    ...
    let currentYear = (new Date()).getFullYear();
    year = year ? year : currentYear;
    let id = fetch(encodeURI('http://www.omdbapi.com/?t=' + movieItem.title +
            '&y=' + year +
            '&apikey=' + API_KEY))
        .then(response => response.json())
        .then(json => json.imdbID)
        .catch((error) => {
            console.log(`Error while getting IMDB id (API approach) for ${movieItem.title}: ${err}`);
            return null;
        });
